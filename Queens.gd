...
func validate_queen(coordinates: Vector2i, position_to_piece: Dictionary):
	var piece: global.PieceSpec = position_to_piece[coordinates]
	
	if piece.type != global.PIECE.QUEEN:
		return
...
func validate(
	...
	var pieces: Array = PuzzleUtils.extract_piece_positions(position_to_piece, global.PIECE.QUEEN)
	
	var valid = not pieces.is_empty() and pieces.all(
		func passes_rule(key: Vector2i):
			return validate_queen(key, position_to_piece) == true
	)
